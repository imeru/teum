...
- `tasks[]`: `{id,title,notes,status,priority(1~4),tags[],projectId,due,dueTime,repeat,block,weight,createdAt,updatedAt,completedAt}`
  - `weight`: `'light' | 'focus' | null` — 에너지/집중도. '지금 이 틈' 추천의 energyFit 가중에 사용(비영속 에너지 모드와 결합).
  - `repeat`: `'daily' | 'weekly' | 'monthly' | null` — 반복 할 일. 완료 시 다음 회차(마감 +1일/+7일/+1개월, 말일 클램프)가 자동 생성(nextRepeatDate).
