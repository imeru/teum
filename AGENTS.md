...
7. **순수 함수는 `logic.js`(도메인)·`helpers.js`(범용)에, 상태·이벤트·렌더는 `app.js`(단일 IIFE)에 둔다.** `logic.js`는 DOM·window·navigator에 의존하면 안 된다(테스트가 직접 호출하는 순수 도메인 계층). `helpers.js`는 범용 계층이라 DOM 생성 헬퍼(`$`, `el`)와 플랫폼 감지(`isStandalone` 등)를 포함해도 된다.
...
2. `npm test` 전부 통과(수백 개, 실행 결과의 N/N passed가 기준). **새 동작에는 테스트를 추가**하고, 기존 테스트를 약화시키지 않는다.
...
- **타임박스(plan)**: 좌측 미니월달력+TOP3+할 일 풀('오늘 할 일'/'다음 할 일 (GTD)', 그룹 개수는 미완료만)+일정 목록, 우측 일간/주간/월간 캘린더.
  - 일간: 06~24시(기본 08시 시작, 새벽 접기), 10분 스냅, 1분=1px. 드래그 배치·이동·길이 조절, 완료 체크(취소선), 겹침은 나란히(assignLanes), 블록 높이 44px 미만은 한 줄(compact)·22px 미만은 제목만(tiny — 1분=1px이므로 대략 40분·20분). 주간: 요일별 세로 캘린더. 월간: 종일·멀티데이 배너, 공휴일 이름 표시(다년 자동).
...
## 코드 컨벤션
- DOM 생성 `el(html)`, 이스케이프 `esc()`(사용자 입력은 반드시), 선택 `$()`. 아이콘은 index.html 스프라이트의 `<use href="#i-이름">`을 `svgIco()`(16px)/`cic()`(13px 칩용)로.
